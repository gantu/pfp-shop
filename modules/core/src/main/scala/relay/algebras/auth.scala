...
import relay.domain.auth.UserName
import relay.domain.auth.Password
import relay.http.json._
import relay.effects.{ GenUUID, MonadThrow }
import relay.config.data.TokenExpiration
...
object LiveAuth {
  def make[F[_]: Sync](
      ...
      users: Users[F],
      redis: RedisCommands[F, String, String]
  ): F[Auth[F]] = Sync[F].delay(new LiveAuth(tokenExpiration, tokens, users, redis))
}
...
final class LiveAuth[F[_]: GenUUID: MonadThrow] private (
  ...
  def logout(token: JwtToken, username: auth.UserName): F[Unit] = redis.del(token.value) *> redis.del(username.value)
}
